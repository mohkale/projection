...
(require 'projector-find)

(setq python-indent-guess-indent-offset-verbose nil)

(defun projector-find-test--setup-project-tree (file-tree &optional base)
           ...
           when (stringp file)
             ...
             if (consp file)
               do (let ((sub (f-join base (car file))))
                    (mkdir sub)
                    (projector-find-test--setup-project-tree (cdr file) sub))
             else
               do (error "Unexpected argument type" file)))

(defun projector-find-test--setup-project (file-tree)
  "Test helper to setup a project file-tree and then create a git project from it.
See `projector-find-test--setup-project-tree' for a description of FILE-TREE."
  (projector-find-test--setup-project-tree file-tree)
  (expect
   (call-process-shell-command "git init && git add -A && git commit -m \"Initial commit\"")
   :to-equal 0))

(describe "Projector find other file"
  ...
  (before-each
    ...
    ;; `projector-types' from other tests will impact the tests in this module. To
  ...
  (it "Can jump between related files in a project based on file extension"
    ...
    (projector-find-test--setup-project
     '("Makefile"
       ("src"
        "foo.h"
        "foo.cpp"
    ...
    ;; WHEN
    ;;   I run `projector-find-other-file'.
    (projector-find-other-file)
    ;; THEN
    ...
    (projector-find-test--setup-project
     '("requirements.txt"
       ...
       ("test"
        "test_foo.py"
        "foo_test.py")))
    (expect (car (projector-project-type default-directory)) :to-equal 'python-pip)
  ...
  (it "Cannot jump between files not in a project"
    ...
    (let ((err (should-error (projector-find-other-file))))
